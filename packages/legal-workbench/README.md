...
The Sources screen accepts pasted text plus PDF, PNG/JPEG, HTML, and DOCX uploads. Visual ingestion runs the pinned local Docling worker in adaptive or strict-visual mode and preserves canonical page images and exact regions. Development uses Tesseract; Electron releases bundle a manifest-validated managed Python runtime with offline Docling models and RapidOCR/PyTorch Latin-script OCR. HTML and DOCX use an inert structural Docling path with stable section paths and character offsets. A completed source can be reprocessed into another immutable representation without replacing prior evidence. Failed or partial work stays out of model context. A missing or invalid installed resource is reported as unavailable and disables document upload/reprocessing rather than searching the network or pretending the worker is present.

The Sources screen also accepts public HTTP(S) URLs. Structural mode uses bounded redirects and response size, rejects local/private/reserved targets, archives requested/final/canonical URL metadata, and parses the stored HTML inertly. Packaged desktop supplies a supervised hidden renderer using its already-shipped isolated Electron Chromium; the HTML and separately hashed full-page screenshot are parsed into structural and visual OCR representations under one source version. A standalone workbench without that authenticated loopback service fails closed. The renderer blocks private targets, writes, frames, active request channels, permissions, downloads, and persistent browser storage. DNS connection pinning and the reviewed live-web corpus remain production gates.
