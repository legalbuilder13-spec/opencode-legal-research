...
The fork still needs its own protected production release workflow and signing identities. Counsel must approve the dependency/model receipts. After those external prerequisites exist, build the final signed/notarized artifacts, run the attestations, and independently verify the downloaded bytes, platform signatures, SBOM attestation, and build provenance. Until that succeeds, signed/notarized installer packaging remains pending.
