...
This milestone does not approve the licenses. Both receipts remain visibly `pending-counsel-review`; the release-only approval mode therefore fails intentionally. Checked-in policy records now require a reviewer, timestamp, and durable review reference before accepting `approved`. Counsel must confirm compatibility, notices, model/training-data rights, and two exact-version source-repository overrides before a release candidate can pass the approval gate.
